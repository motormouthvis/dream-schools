...
const MAP_COOKIE = "dse_embed_map";
...
export function NearbySchoolsFull({
  ...
  // Remember the map on/off choice across visits (cookie).
  useEffect(() => {
    if (readCookie(MAP_COOKIE) === "0") setShowMap(false);
  }, []);
  function toggleMap() {
    setShowMap((v) => {
      const next = !v;
      writeCookie(MAP_COOKIE, next ? "1" : "0");
      return next;
    });
  }
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      <div className="mb-3 flex shrink-0 flex-wrap items-center gap-x-3 gap-y-1">
        ...
        <div className="ml-auto flex flex-wrap items-center gap-1.5">
          ...
          <button
            ...
            className={`ml-1.5 inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-semibold transition ${
              showMap ? "bg-brand-600 text-white shadow-sm" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
            }`}
          >
            <MapPinIcon />
            Map
      ...
      {/* List + map together, filling the fixed height. When the map is off,
          the schools fill the width in two scrollable columns. */}
      <div className={`grid min-h-0 flex-1 grid-cols-1 gap-4 ${showMap ? "lg:grid-cols-2" : ""}`}>
