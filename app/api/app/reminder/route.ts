...
export async function POST(request: Request) {
  const me = await currentUser(request);
  if (!me) return NextResponse.json({ error: "Not signed in." }, { status: 401 });
  let body: { intervalDays?: number; send?: boolean; includeAll?: boolean };
  try {
  ...
  if (body.send) {
    const result = await sendCustomerReminder(me.id, { manual: true, includeAll: body.includeAll !== false });
