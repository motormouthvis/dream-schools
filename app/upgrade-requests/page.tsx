...
function UpgradeRequests({ isOwner, isPartner, email }: { isOwner: boolean; isPartner: boolean; email: string }) {
  ...
  const [reminderUpgraded, setReminderUpgraded] = useState(false);
  ...
  const [historyKind, setHistoryKind] = useState<"all" | "offer" | "reminder">("all");
  const [historySortKey, setHistorySortKey] = useState<"sentAt" | "customer" | "kind">("sentAt");
    ...
    try {
      ...
      if (!isManager) {
        ...
        if (remRes.ok && remJson.settings) {
          ...
          setReminderMaxDays(remJson.settings.maxDays || 30);
          setReminderUpgraded(Boolean(remJson.settings.upgraded));
        }
      }
    } catch {
  ...
  const historyScoped = scopeType !== "all" && Boolean(scopeId);
  const filteredOfferEmails = useMemo(() => {
    let rows = offerEmails;
    // Respect the View/Select selection.
    if (scopeType === "customer" && scopeId) rows = rows.filter((e) => e.customerId === scopeId);
    else if (scopeType === "partner" && scopeId) rows = rows.filter((e) => e.partnerId === scopeId || e.customerId === scopeId);
    if (historyKind !== "all") rows = rows.filter((e) => e.kind === historyKind);
    const q = offerHistoryFilter.trim().toLowerCase();
    if (q) rows = rows.filter((e) => `${e.customerName} ${e.customerEmail}`.toLowerCase().includes(q));
    const dir = historySortDir === "asc" ? 1 : -1;
    return [...rows].sort((a, b) => {
      if (historySortKey === "customer") return (a.customerName || a.customerEmail).localeCompare(b.customerName || b.customerEmail) * dir;
      if (historySortKey === "kind") return a.kind.localeCompare(b.kind) * dir;
      return (new Date(a.sentAt).getTime() - new Date(b.sentAt).getTime()) * dir;
    });
  }, [offerEmails, scopeType, scopeId, historyKind, offerHistoryFilter, historySortKey, historySortDir]);
  ...
  function historySortButton(key: "sentAt" | "customer" | "kind", label: string) {
    const active = historySortKey === key;
    return (
      <button
        onClick={() => {
          if (historySortKey === key) setHistorySortDir((d) => (d === "asc" ? "desc" : "asc"));
            ...
            setHistorySortDir(key === "sentAt" ? "desc" : "asc");
          }
        }}
        className="inline-flex items-center gap-1 uppercase tracking-wide hover:text-slate-700"
      >
        {label}
        {active && <span className="text-[9px]">{historySortDir === "asc" ? "▲" : "▼"}</span>}
      </button>
    );
  }
          ...
          <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] text-slate-400">
                {reminderUpgraded ? "(any cadence — you're on Neighborhood Explorer)" : "(max 30 days)"}
              </span>
            </div>
        ...
        <div className="mt-6 rounded-xl border border-slate-200 bg-white p-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <button onClick={() => setOfferHistoryOpen((o) => !o)} className="flex items-center gap-2 text-left">
              <span className="text-lg font-extrabold text-ink-900">Sent Email Reminder/Offer History</span>
          ...
          {!offerHistoryOpen ? (
            <p className="mt-1 text-[12px] text-slate-500">A record of reminder and special-offer emails you've sent. Click to expand.</p>
          ) : (
          <>
          {historyScoped && (
            <p className="mt-2 rounded-lg bg-amber-50 px-3 py-2 text-[12px] text-amber-800">
              Showing history for the selected {scopeType === "partner" ? "partner" : "realtor"} only. Select <strong>All</strong> in View/Select above to see everything.
            </p>
          )}
          <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
            <div className="inline-flex overflow-hidden rounded-lg border border-slate-200 text-xs font-bold">
              {(["all", "reminder", "offer"] as const).map((k) => (
                <button
                  key={k}
                  onClick={() => setHistoryKind(k)}
                  className={`px-3 py-1.5 transition ${historyKind === k ? "bg-brand-600 text-white" : "bg-white text-slate-600 hover:bg-slate-50"}`}
                >
                  {k === "all" ? "All" : k === "reminder" ? "Reminder" : "Special Offer"}
                </button>
              ))}
            </div>
            <PageSizeButtons value={offerHistoryPageSize} onChange={setOfferHistoryPageSize} />
          </div>
          <div className="mt-2 overflow-x-auto rounded-xl border border-slate-200">
            <table className="min-w-full text-left text-sm">
              <thead className="border-b border-slate-200 bg-slate-50 text-[11px] uppercase tracking-wide text-slate-500">
                <tr>
                  <th className="px-3 py-2 font-semibold">{historySortButton("sentAt", "Sent")}</th>
                  <th className="px-3 py-2 font-semibold">{historySortButton("kind", "Type")}</th>
                  <th className="px-3 py-2 font-semibold">{historySortButton("customer", "Recipient")}</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredOfferEmails.length === 0 ? (
                  <tr><td colSpan={isOwner ? 8 : 7} className="px-3 py-6 text-center text-slate-400">No emails yet.</td></tr>
                ) : (
                  (offerHistoryPageSize === 0 ? filteredOfferEmails : filteredOfferEmails.slice(0, offerHistoryPageSize)).map((email) => (
                    <tr key={email.id}>
                      ...
                      <td className="px-3 py-2.5">
                        <span className={`rounded-full px-2 py-0.5 text-[11px] font-bold ${email.kind === "offer" ? "bg-amber-100 text-amber-800" : "bg-slate-100 text-slate-600"}`}>
                          {email.kind === "offer" ? "Special Offer" : "Reminder"}
                        </span>
                      </td>
                      <td className="px-3 py-2.5">
                      ...
                      <td className="whitespace-nowrap px-3 py-2.5 text-right">
                        <button onClick={() => setEmailPreview({ subject: `${email.kind === "offer" ? "Special Offer" : "Reminder"} · ${email.customerName || email.customerEmail}`, html: email.html })} className="rounded-md border border-slate-300 px-2.5 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-100">View</button>
                      </td>
                    </tr>
                  ))
                )}
